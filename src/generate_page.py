...
def generate_pages_recursive(from_path="./content", template_path="./template.html", basepath="/"):
    for path in os.listdir(from_path):
        path = os.path.join(from_path, path)
        from_new = path
        dest_new = path.replace("/content", "/docs")
        dest_new = dest_new.replace(".md", ".html")

        if os.path.isdir(path):
            generate_pages_recursive(from_new, template_path, basepath)
        elif path.endswith(".md"):
            generate_page(from_new, template_path, dest_new, basepath)
